...
* @description
*/
public class SaleWriteoffListController extends SaleListCellController{
	 ...
	 * @param vo
	 */
	public SaleWriteoffListController(ListViewController stockListViewController, SalesmanListVO vo,String id) {
		super(stockListViewController, SaleBLFactory.getSaleListBLService().get(id));
	}
	...
	void save(){
		...
		Platform.runLater(()->{
		try {
   		 SaleWriteoffEditController controller = new SaleWriteoffEditController(null,SaleBLFactory.getSaleListBLService(),vo.getId(),vo);
	...
	public SalesmanListVO getMinusVOFromUI() {
		return new SaleListVO(id,operator.getText(),operatorId, null,operatorGrade,VIPID.getText(),
				VIPName.getText(), grade,null, "默认仓库", notesTextField.getText(), WriteoffHelper.getMinus(chosenList),
